...
const Header: React.FC<HeaderProps> = ({ onBookClick, onHomeClick, onAccountClick, onMasterClick, onAdminClick }) => {
  ...
  const navLinks = [
    { name: 'Главная', href: '#', action: onHomeClick },
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <button 
          onClick={onHomeClick}
          className="text-2xl font-rounded font-bold tracking-tight text-[#4A3728] outline-none"
        >
          Khan's Art
        </button>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a 
              ...
              className="text-[#4A3728] hover:text-[#8B6F5C] font-medium transition-colors"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#F5F0E8] border-t border-[#E8C4B8] shadow-xl p-6 flex flex-col space-y-4 animate-in slide-in-from-top">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href}
              className="text-[#4A3728] text-lg font-medium py-2 border-b border-[#E8C4B8]"
              onClick={(e) => {
                setIsMenuOpen(false);
                if (link.action) {
                  e.preventDefault();
